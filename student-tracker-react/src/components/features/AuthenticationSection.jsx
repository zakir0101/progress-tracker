...
function AuthenticationSection({ onSignIn }) {
  const loginWithGoogle = useGoogleLogin({
    onSuccess: (codeResponse) => {
      console.log('Google OAuth successful:', codeResponse)
      if (onSignIn) {
        onSignIn(codeResponse)
      }
    },
    onError: () => {
      console.error('Google OAuth failed')
    },
    flow: 'implicit', // Using implicit flow for JWT tokens
    ux_mode: 'popup',
  })

  return (
    <div className="bg-blue-50 p-5 rounded-lg mx-5 my-5 text-center border-l-4 border-blue-500 flex flex-col justify-center">
